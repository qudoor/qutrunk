...
class PaddleLayer(Layer):
    ...
    # class _PaddleNNFunction(Function):
    #     @staticmethod
    #     def forward(ctx: Any, input_data: Any, neural_network: NeuralNetwork):
    ...
    def forward(self, input_data: Any) -> Any:
        input_ = input_data if input_data is not None else paddle.to_tensor([])
        #if self._neural_network.gradient == 'auto':
        return self._neural_network.forward(input_)
        
        #return PaddleLayer._TorchNNFunction.apply(input_, self._neural_network)
